...
def append_dict(dict_object, nct, patient, date):
    dict_object["nct"].append(nct)
    dict_object["patient"].append(patient)
    dict_object["date"].append(date)
    return dict_object


def main(formatted_data: pd.DataFrame):

    FP_trials_data = {"nct": [], "patient": [], "date": []}
    FN_trials_data = {"nct": [], "patient": [], "date": []}

    for _, patient_row in formatted_data.iterrows():
        patient_id = patient_row["patient_id"]
        patient_trials = patient_row["nct_id"]
        date = patient_row["analysis_date"]
        for nct in FP_trials:
            if nct in patient_trials:
                append_dict(FP_trials_data, nct, patient_id, date)
        for nct in FN_trials:
            if nct in patient_trials:
                append_dict(FN_trials_data, nct, patient_id, date)
